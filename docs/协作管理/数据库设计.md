...
6. 补发必须保留原 `certificate_id`，生成新的 `certificate_no`、PDF、哈希和回执，并将状态更新为 `REISSUED`。
...
| 表 | 关键字段 | 约束与用途 |
| --- | --- | --- |
| `users` | `user_id`、`username`、`display_name`、`password_hash`、`role`、`student_id`、`must_change_password`、`is_active` | 用户名唯一；`student_id` 可空且唯一外键关联 `students`；密码仅保存 Argon2 哈希；禁用账号后不可继续登录 |
...
| `auth_sessions` | `jti`、`user_id`、`expires_at`、`revoked_at` | JWT 通过 `jti` 关联可撤销会话；登出、禁用、重置密码或首次改密时会写入 `revoked_at` |

角色范围：`ADMIN` 管理账号、教师邀请及学生账号开通/重置；`TEACHER` 处理证书业务；`AUDITOR` 仅只读存证与审计数据；`STUDENT` 只能访问其 `student_id` 绑定的证书。学生端身份不由 `student_no` 查询参数决定。

## 8. 撤销与补发数据一致性

### 8.1 撤销旧证书

执行撤销前，证书当前状态必须为 `VALID` 或 `REISSUED`。撤销操作必须同时完成：
...
补发不再新增第二条 certificates 记录，而是在原 certificate_id 上更新可验证内容：

| 字段 | 规则 |
| --- | --- |
| certificate_id | 保持不变 |
| certificate_no | 重新生成，不复用被撤销编号 |
| pdf_path / qr_code_path | 重新生成 PDF 与二维码 |
| verify_url | 指向新的证书编号 |
| certificate_hash | 对新 PDF 重新计算 SHA-256 |
| receipt_id | 生成新回执并切换为当前回执 |
| status | 从 `REVOKED` 更新为 `REISSUED`，不恢复为 `VALID` |

旧回执、撤销记录与审计日志仍保留，审计日志记录补发前后的证书编号。管理端证书列表和详情直接返回记录中的真实 `REISSUED` 状态，不需要前端推断。补发后的记录可再次执行撤销；状态闭环为 `REVOKED` → `REISSUED` → `REVOKED`，且每次回到 `REVOKED` 后均可再次补发。
### 8.3 验真判断顺序
...
5. 检查并直接返回证书当前真实状态；`VALID` 与 `REISSUED` 均表示未撤销，但不得把 `REISSUED` 映射成 `VALID`。
6. 根据以上结果返回明确失败原因。
