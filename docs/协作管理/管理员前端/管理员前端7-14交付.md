...
| `GET /api/admin/certificate-batches` | `current`、`size`、`keyword`、`status` | 分页；记录含 `batch_id`、`batch_no`、`batch_name`、`project_name`、`template_id`、`student_count`、`generated`、`evidenced`、`status` |
| `POST /api/admin/certificate-batches` | `batch_name`、`project_name`、`template_id`、`status` | 新增后的批次对象 |
| `PUT /api/admin/certificate-batches/{batch_id}` | 可编辑批次字段 | 更新后的批次对象 |
| `DELETE /api/admin/certificate-batches/{batch_id}` | 路径参数 `batch_id` | 可为空 |
