...
export async function createBatch(data:BatchCreateRequest){if(useMock){await wait();batches.unshift({batch_id:Date.now(),batch_no:`BATCH-${Date.now()}`,batch_name:data.batch_name,project_id:data.project_id,project_name:data.project_name||'',template_id:data.template_id,student_count:data.student_ids.length,generated:0,evidenced:0,status:'DRAFT'});return}await request.post('/admin/batches',snakeize(data))}
