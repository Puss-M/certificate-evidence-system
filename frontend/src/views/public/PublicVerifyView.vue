...
const sampleCases = [
  { label: 'PASS', certificateNo: 'CERT-20260713-0001', mode: 'number' },
  { label: 'HASH_MISMATCH', certificateNo: 'CERT-20260713-0001', mode: 'tampered' },
  { label: 'REVOKED', certificateNo: 'CERT-20260710-0003', mode: 'number' },
  { label: 'REISSUED', certificateNo: 'CERT-20260712-0011', mode: 'number' },
  { label: 'NOT_FOUND', certificateNo: 'CERT-NOT-EXIST', mode: 'number' },
  { label: 'NO_RECEIPT', certificateNo: 'CERT-20260713-0002', mode: 'number' },
]
...
const resultMeaning = computed(() => {
  const map: Record<string, string> = {
    PASS: '证书编号存在，状态有效，哈希一致，并且能找到存证回执。',
    HASH_MISMATCH: '证书存在，但上传文件或系统留存文件与存证哈希不一致。',
    REVOKED: '证书已被管理员撤销，第三方不应继续采信。',
    REISSUED: '该证书已补发，验收时应继续查看新的证书编号。',
    NOT_FOUND: '系统内不存在该证书编号，可能输错或伪造。',
    NO_RECEIPT: '证书记录存在，但尚未完成链上或哈希链存证。',
    SYSTEM_ERROR: '验真服务异常，需要保留截图并反馈后端或部署同学。',
  }
  return result.value ? map[result.value.verify_result] || result.value.verify_message : ''
})
const selectedFileName = computed(() => selectedFile.value?.name || '选择 PDF')
...
async function runSample(item: { certificateNo: string; mode: string }) {
  certificateNo.value = item.certificateNo
  if (item.mode === 'tampered') {
    selectedFile.value = new File(['tampered demo'], 'tamper-demo.pdf', { type: 'application/pdf' })
    await verifyFile()
    return
  }
    ...
    <main class="verify-main">
      <section class="verify-workbench">
        ...
        <div class="verify-panel">
          ...
          <div v-if="useMock" class="sample-row">
            <span>验收测试样例</span>
            <el-button v-for="item in sampleCases" :key="item.label" size="small" plain @click="runSample(item)">{{ item.label }}</el-button>
          </div>
        </div>
      </section>
      ...
      <section v-if="result" class="verify-result panel" :class="resultTone">
        ...
        <div class="result-grid">
          ...
          <div><span>补发证书</span><b>{{ result.new_certificate_no || '—' }}</b></div>
        </div>
        <div class="test-flags">
          <div><span>编号/扫码验真</span><b>{{ result.verify_result === 'NOT_FOUND' ? '未通过' : '已完成' }}</b></div>
          <div><span>PDF 复验</span><b>{{ result.uploaded_hash ? '已上传' : '未上传' }}</b></div>
          <div><span>状态结论</span><StatusTag :value="result.verify_result" /></div>
        </div>
        <dl class="hash-list">
          <dt>系统存证哈希</dt><dd>{{ result.stored_hash || result.certificate_hash || '—' }}</dd>
          <dt>上传文件哈希</dt><dd>{{ result.uploaded_hash || `未上传 PDF：当前${selectedFileName}` }}</dd>
