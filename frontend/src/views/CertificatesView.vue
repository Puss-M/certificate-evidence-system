...
import { deleteCertificate, downloadAdminCertificate, evidenceCertificate, getCertificates, reissueCertificate, revokeCertificate } from '@/api/certificates'
...
async function download(row:Certificate){if(!row.pdf_path)return ElMessage.warning('该证书尚未生成 PDF，暂时无法下载');const pdf=await downloadAdminCertificate(row.certificate_no);const url=URL.createObjectURL(pdf);const link=document.createElement('a');link.href=url;link.download=`${row.certificate_no}.pdf`;link.click();URL.revokeObjectURL(url)}
