...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/public/verify/:certificateNo', component: () => import('@/views/public/PublicVerifyView.vue'), meta: { public: true, title: '证书验真' } },
    {
      ...
      redirect: () => roleHome(useAuthStore().user?.role),
      children: [
...
export function canRoleAccessPath(path: string, role?: Role) {
  if (!path.startsWith('/') || !role) return false
  const resolved = router.resolve(path)
  if (!resolved.matched.length || resolved.matched.some(record => record.path.includes(':pathMatch'))) return false
  return resolved.matched.every(record => {
    const roles = record.meta.roles as Role[] | undefined
