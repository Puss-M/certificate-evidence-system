...
3. **配置 `backend/.env`**（照抄 `.env.example`）：

   ```
   CHAIN_RPC_URL=http://127.0.0.1:8545
   CHAIN_BACKEND_PRIVATE_KEY=<第2步拿到的私钥>
   CHAIN_CONTRACT_ADDRESS=
   CHAIN_EXPECTED_CHAIN_IDS=1337,31337
   ```

   `CHAIN_CONTRACT_ADDRESS` 先留空，等第4步部署完再填。`CHAIN_EXPECTED_CHAIN_IDS`
   是安全白名单，默认只允许 Ganache/Hardhat 常用本地链 ID；不要加入真实主网链 ID。
...
5. 重启后端。之后每次调用批次生成 Merkle Root 的接口
   （`POST /api/admin/batches/{batch_id}/merkle-root`），只要三项连接配置都齐全、
   链能连接且链 ID 在允许列表内，返回结果和数据库里 `credential_roots.tx_hash` 字段就会带上链上
